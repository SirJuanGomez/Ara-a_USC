...
class Control:
    ...
    def condition_monitor(self):
        while True:
            if (time.time() - self.timeout) > 10 and self.timeout != 0 and self.command_queue[0] == '':
                ...
                self.relax(True)
                self.status_flag = 0x00
            # Using direct string comparison instead of cmd constants
            elif self.command_queue[0] == 'position' and len(self.command_queue) == 4:
                ...
                self.command_queue = ['', '', '', '', '', '']
            elif self.command_queue[0] == 'attitude' and len(self.command_queue) == 4:
                ...
                self.command_queue = ['', '', '', '', '', '']
            elif self.command_queue[0] == 'move' and len(self.command_queue) == 6:
            ...
            elif self.command_queue[0] == 'balance' and len(self.command_queue) == 2:
                if self.command_queue[1] == "1":
                    ...
                    self.imu6050()
            elif self.command_queue[0] == 'calibration':
                self.timeout = 0
                self.calibrate()
                self.set_leg_angles()
    ...
    def run_gait(self, data, Z=40, F=64):  # Example: data=['move', '1', '0', '25', '10', '0']  Changed 'CMD_MOVE' to 'move'
        gait = data[1]
